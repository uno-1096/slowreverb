...
function detectBPM(buffer: AudioBuffer): number {
  ...
  for (let i = 1; i < energies.length - 1; i++) {
    if (energies[i] > mean * 3.0 && energies[i] > energies[i - 1] && energies[i] > energies[i + 1]) {
      beats.push(i);
    }
  }
...
export default function AudioPlayer({ file }: Props) {
  ...
  // Store raw IR ArrayBuffers so they can be decoded into any AudioContext
  const irRawRef = useRef<Record<string, ArrayBuffer>>({});
  ...
  useEffect(() => {
    ...
    const irLoads = PRESETS.filter(p => p.file).map(p =>
      fetch(p.file!).then(r => r.arrayBuffer())
        .then(ab => ({ id: p.id, ab })).catch(() => null)
    );
    ...
    Promise.all([decodeMain, Promise.all(irLoads)]).then(([decoded, irs]) => {
      ...
      // Store raw bytes
      irs.forEach(r => { if (r) irRawRef.current[r.id] = r.ab; });
      // Decode into main ctx for preview
      const decodeIrs = irs.map(r => r
        ? ctx.decodeAudioData(r.ab.slice(0)).then(buf => ({ id: r.id, buf })).catch(() => null)
        : null
      );
      Promise.all(decodeIrs).then(decoded2 => {
        const irMap: Record<string, AudioBuffer> = {};
        decoded2.forEach(r => { if (r) irMap[r.id] = r.buf; });
        setIrBuffers(irMap);
      });
  ...
  // For export: decode IR into OfflineAudioContext
  const getImpulseForOffline = useCallback(async (
    ctx: OfflineAudioContext, preset: string, decay: number
  ): Promise<AudioBuffer> => {
    const raw = irRawRef.current[preset];
    if (raw) {
      try { return await ctx.decodeAudioData(raw.slice(0)); } catch {}
    }
    // Fallback synthetic
    const sr = ctx.sampleRate;
    const len = Math.floor(sr * decay);
  ...
  const buildGraph = useCallback((
    ...
    convolver.buffer = irOverride ?? getImpulse(ctx, fx.reverbPreset, fx.reverbDecay);
  ...
  const handleCanvasMouseDown = (e: React.MouseEvent) => {
    if (e.shiftKey) {
      ...
      if (isPlaying) {
        ...
        drawWaveform(bufferRef.current, loopRegion, zoom, zoomOffset, t);
      }
    }
  };
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // (offline rendering happens server-side via FFmpeg)
